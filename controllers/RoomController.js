...
async function processRooms({ req, found, mutateRoomInfo }) {
    ...
    let allMessages = await find({ col: "messages", query: { room: { $in: foundRoomTokens } }, filter: { msg: 1, room: 1, updatedAt: 1, createdAt: 1, userId: 1, isRead: 1, isDeleted: 1, img: 1, name: 1, _id: 0 } })
    ...
    let foundInfoWithRoomToken = foundInfo.map((info, infoInd) => ({
        ...
        createdAt: lastMsgsArr?.[infoInd]?.createdAt,
