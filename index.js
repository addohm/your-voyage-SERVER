...
app.post("/deleteMessage", (req, res, next) => whoCanPass({ req, res, next, role: "user" }), MessageController.deleteMessage, (req, res, next) => io.to(req.room).emit('delete_message', { _id: req._id, updatedAt: req.updatedAt }))